...
import {
  ...
  STACKED_AVATAR_BORDER_WIDTH,
} from "./Facepile.constants";

/**
 * Used to sort Avatar arrays by putting active Avatars before inactive Avatars
 * Used for inline Facepiles
 */
export function compareInlineAvatars(
  firstItem: AvatarItem,
  secondItem: AvatarItem
) {
  return firstItem.props?.active === secondItem.props?.active
    ? 0
    : firstItem.props?.active
    ? -1
    : 1;
}
 ...
 */
export function compareStackedAvatars(
  firstItem: AvatarItem,
  secondItem: AvatarItem
) {
  return firstItem.props?.active === secondItem.props?.active
 ...
 */
export const getStackedFacepileWrapperSize = (
  numOfItems: number,
  size: FacepileSize
) => {
  const avatarSize = AVATAR_SIZE_STYLES[size].avatarSize;
  const avatarWrapperSize =
    numOfItems <= 1 ? avatarSize : avatarSize + STACKED_AVATAR_BORDER_WIDTH * 2;
...
export const getStackedAvatarPositions = (
  ...
  switch (numOfItems) {
    case 2: {
      if (index === 1)
        return { left: avatarWrapperSize * 0.5, top: avatarWrapperSize * 0.5 };
      return { left: 0, top: 0 };
    }
    case 3: {
      if (index === 1)
        return { left: avatarSize * 0.75, top: avatarSize * 0.5 };
    ...
    case 4: {
      ...
      if (index === 3)
        return { left: avatarWrapperSize, top: avatarWrapperSize }; // Position of the More label
      return { left: 0, top: 0 };
    }
    case 1:
    default:
